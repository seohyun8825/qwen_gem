...
def _ensure_attn_implementation(attn_implementation: str) -> str:
    """Attempt to ensure requested attention implementation is available.

    If FlashAttention is requested but missing, try to install a prebuilt wheel.
    Falls back to "eager" when installation/import fails.
    """

    if attn_implementation not in {"flash_attention_2", "flash"}:
    ...
    global _FLASH_ATTEMPTED
    if not _FLASH_ATTEMPTED:
        _FLASH_ATTEMPTED = True
        try:
            from huggingface_hub import hf_hub_download  # type: ignore

            py_ver = f"cp{sys.version_info.major}{sys.version_info.minor}"
            wheel_name = f"128/flash_attn_3-3.0.0b1-{py_ver}-abi3-linux_x86_64.whl"
            rank0_print(f"Attempting to download FlashAttention wheel {wheel_name}...")
            ...
            rank0_print("FlashAttention installation attempt finished.")
        except Exception as exc:
            warnings.warn(f"FlashAttention installation failed: {exc}. Falling back to eager attention.")

    if _flash_available():
        return attn_implementation

    warnings.warn("FlashAttention still unavailable. Using eager attention implementation.")
    return "eager"
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
    ...
    attn_implementation = _ensure_attn_implementation(attn_implementation)
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    config_source = model_base if model_base is not None else model_path
                    llava_cfg = LlavaQwenConfig.from_pretrained(config_source)
                    if overwrite_config is not None:
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                    ...
                    model_load_path = model_base if model_base is not None else model_path
                    model = LlavaQwenForCausalLM.from_pretrained(model_load_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
            ...
            if "mixtral" in model_name.lower():
                ...
                saved_cfg = LlavaQwenConfig.from_pretrained(model_path)
                if hasattr(saved_cfg, "compressor_type"):
                    llava_cfg.compressor_type = saved_cfg.compressor_type
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if image_processor is None:
            ...
            if vision_tower_name is not None:
                try:
                    processor = AutoImageProcessor.from_pretrained(vision_tower_name)
                    ...
                    class _Wrapper:
                        ...
                        def preprocess(self, video, return_tensors="pt"):
                            return self.proc(video, return_tensors=return_tensors)

                    image_processor = _Wrapper(processor)
                except Exception as exc:
                    warnings.warn(
                        f"Failed to load AutoImageProcessor for {vision_tower_name}: {exc}"
                    )
                    image_processor = None
            if image_processor is None:
                vision_tower_name = getattr(model.config, "mm_vision_tower", None)
                if vision_tower_name is not None:
                    try:
                        ...
                        warnings.warn(f"Failed to load AutoImageProcessor for {vision_tower_name}: {exc}")
                        image_processor = None
